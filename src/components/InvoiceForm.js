...
class InvoiceForm extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      currentDate: '',
      invoiceNumber: 1,
      dateOfIssue: new Date().toLocaleDateString('es-ES'),
      billTo: '',
      billToNIF: '',
      billToAddress: '',
      billFrom: '',
  ...
  launchEnvVars() {
    const defaultFromData = process.env.REACT_APP_DEFAULT_DATA_FORM.split('|');
    console.log("launch env vars ", defaultFromData);
    this.setState({
      billFrom: defaultFromData[0]
      });
    this.setState({billFromNIF: defaultFromData[1]
      });
    this.setState({
      billFromAddress: defaultFromData[2]});
  }
  handleCalculateTotal() {
    ...
    this.setState({
      ...
      this.setState({
        irpf: parseFloat(parseFloat(subTotal) * (this.state.irpfRate / 100)).toFixed(2)
      }, () => {
        this.setState({
          ...
          this.setState({
            ...
            this.setState({
              total: parseFloat((subTotal - this.state.discountAmmount) - parseFloat(this.state.irpf) + parseFloat(this.state.taxAmmount)).toFixed(2)
            });
          });
        });
      });
          ...
          <Card className="p-4 p-xl-5 my-3 my-xl-4">
              ...
              <div class="d-flex flex-column">
                <div className="d-flex flex-column">
                  <div class="mb-2">
                    ...
                    <span className="current-date">{new Date().toLocaleDateString('es-ES')}</span>
                  </div>
                </div>
            ...
            <Row className="mb-5">
              <Col>
                ...
                <Form.Control placeholder={"NIF client"} value={this.state.billToNIF} type="text" name="billToNIF" className="my-2" onChange={(event) => this.editField(event)} autoComplete="email" required="required"/>
              ...
              <Col>
                ...
                <Form.Control placeholder={"NIF"} value={this.state.billFromNIF} type="text" name="billFromNIF" className="my-2" onChange={(event) => this.editField(event)} autoComplete="NIF" required="required"/>
              ...
              <Col lg={6}>
                <div className="d-flex flex-row align-items-start justify-content-between">
                  ...
                  <span>{this.state.currency}
                    {this.state.subTotal || 0}</span>
                </div>
                <div className="d-flex flex-row align-items-start justify-content-between mt-2">
                  <span className="fw-bold">IRPF:
                  </span>
                  <span>-{this.state.currency}
                    {this.state.irpf || 0}</span>
                </div>
                <div className="d-flex flex-row align-items-start justify-content-between mt-2">
            ...
            <Form.Label className="fw-bold">Payment account:</Form.Label>
            <Form.Label className="my-2">{this.state.account}</Form.Label>
          </Card>
        </Col>
        <Col md={4} lg={3}>
          <div className="sticky-top pt-md-3 pt-xl-4">
            ...
            <Form.Group className="my-3">
              ...
              <InputGroup className="my-1 flex-nowrap">
                <Form.Control name="taxRate" type="number" value={this.state.taxRate} onChange={(event) => this.editField(event)} className="bg-white border" placeholder="0.0" min="0.00" step="1" max="100.00"/>
            ...
            <Form.Group className="my-3">
              <Form.Label className="fw-bold">IRPF rate:</Form.Label>
              <InputGroup className="my-1 flex-nowrap">
                <Form.Select name="irpfRate" type="dropdown" value={this.state.irpfRate} onChange={(event) => this.editField(event)} className="bg-white border" placeholder="0">
            ...
            <Form.Group className="my-3">
              ...
              <InputGroup className="my-1 flex-nowrap">
                <Form.Control name="discountRate" type="number" value={this.state.discountRate} onChange={(event) => this.editField(event)} className="bg-white border" placeholder="0.0" min="0.00" step="1" max="21.00"/>
                <InputGroup.Text className="bg-light fw-bold text-secondary small">
                  %
                </InputGroup.Text>
              </InputGroup>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Select name="account" onChange={(event) => this.editField(event)} value={this.state.account} className="btn btn-light my-1" aria-label="Change Account Number">
              <option value="">Seect a bank account...</option>
                  <BankAccounts />
              </Form.Select>
            </Form.Group>
          </div>
